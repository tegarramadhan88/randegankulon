...
L.Control.mouseCoordinate = L.Control.extend({
  options: {
    ...
    _sm_a: 6378137.0,
    _sm_b: 6356752.314,
    _sm_EccSquared: 6.69437999013e-3,
  ...
  _update: function (e) {
    ...
    if (this.options.gps) {
      ...
      if (this.options.gpsLong) {
        ...
        content +=
          "<tr><td></td><td class='coords'>" +
          gpsMinuten.NS +
          " " +
          gpsMinuten.latgrad +
          "&deg; " +
          gpsMinuten.latminuten +
          "</td><td class='coords'> " +
          gpsMinuten.WE +
          " " +
          gpsMinuten.lnggrad +
          "&deg; " +
          gpsMinuten.lngminuten +
          "</td></tr>";
        var gpsMinutenSekunden = this._geo2gradminutensekunden(gps);
        content +=
          "<tr><td></td><td>" +
          gpsMinutenSekunden.NS +
          " " +
          gpsMinutenSekunden.latgrad +
          "&deg; " +
          gpsMinutenSekunden.latminuten +
          "&prime; " +
          gpsMinutenSekunden.latsekunden +
          "&Prime;</td><td> " +
          gpsMinutenSekunden.WE +
          " " +
          gpsMinutenSekunden.lnggrad +
          "&deg; " +
          gpsMinutenSekunden.lngminuten +
          "&prime; " +
          gpsMinutenSekunden.lngsekunden +
          "&Prime;</td></tr>";
      }
    }
    if (this.options.utm) {
    ...
    if (this.options.nac) {
      ...
      content +=
        "<tr><td>NAC</td><td colspan='2'>" + nac.y + " " + nac.x + "</td></tr>";
    }
  ...
  _geo2gradminutensekunden: function (gps) {
    ...
    var latsekunden =
      Math.round((latminuten - parseInt(latminuten, 10)) * 60 * 100) / 100;
    latminuten = parseInt(latminuten, 10);

    var lnggrad = parseInt(gps.lng, 10);
    var lngminuten = (gps.lng - lnggrad) * 60;
    var lngsekunden =
      Math.round((lngminuten - parseInt(lngminuten, 10)) * 60 * 100) / 100;
...
var NAC = {
  ...
  _calcValues: function (z) {
    ...
    ret[3] = parseInt(
      (((z * 30 - ret[0]) * 30 - ret[1]) * 30 - ret[2]) * 30,
      10
    );
    ret[4] = parseInt(
      ((((z * 30 - ret[0]) * 30 - ret[1]) * 30 - ret[2]) * 30 - ret[3]) * 30,
      10
    );
    ret[5] = parseInt(
      (((((z * 30 - ret[0]) * 30 - ret[1]) * 30 - ret[2]) * 30 - ret[3]) * 30 -
  ...
  _nac2Letter: function (number) {
    ...
    if (!isNaN(number) && number < 30) return nac_letters.substr(number, 1);
    else return 0;
  },
};
/**
...
var UTM = {
  ...
  fromLatLng: function (latlng) {
    ...
    var lw = latlng.lng;
    var bw = latlng.lat;
    if (lw === -180) lw += 1e-13; //Number.MIN_VALUE;
    if (lw === 180) lw -= 1e-13; //umber.MIN_VALUE;
  ...
  toLatLng: function (utm) {
    ...
    var f2 =
      ...
      ((3 * ex2) / 8 -
        ...
        (255 * ex8) / 4096);
    var f4 =
      (180 / pi) * ((21 * ex4) / 256 - (21 * ex6) / 256 + (533 * ex8) / 8192);
